...
function getOembed(username) {
    ...
    .then(user => {
        if (user.promotedPost) {
            return buildOembed(user.promotedPost)
        }
        else {
            return fetch(mediaURL + `?fields=permalink&access_token=${user.accessToken}`)
            .then(response => response.json())
