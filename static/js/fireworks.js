...
const store = {
	...
	state: {
		...
		config: {
			quality: QUALITY_LOW, // will be mirrored to a global variable named `quality` in `configDidUpdate`, for perf.
			shell: 'Random',
			size: IS_DESKTOP
				? '3' // Desktop default
				: IS_HEADER 
...
function makePistilColor(shellColor) {
	return (shellColor === COLOR.White || shellColor === COLOR.Gold) ? randomColor({ notColor: shellColor }) : COLOR.Gold;
}
...
const palmShell = (size=1) => {
	...
	while (color == COLOR.White || color == COLOR.Gold) {
		color = randomColor()
	}
...
function randomShellName() {
	return Math.random() < 0.3 ? 'Crysanthemum' : shellNames[(Math.random() * (shellNames.length - 1) + 1) | 0 ];
}
...
function randomShell(size) {
	...
	// Normal operation
	let shell = randomShellName()
	while (shell == "Falling Leaves" || shell == "Willow") {
		shell = randomShellName()
	}
	return shellTypes[shell](size);
}
...
function colorSky(speed) {
	...
	// appNodes.canvasContainer.style.backgroundColor = `rgba(${currentSkyColor.r | 0}, ${currentSkyColor.g | 0}, ${currentSkyColor.b | 0})`;
}
